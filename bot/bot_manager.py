...
async def _run_trading_cycle(exchange, user, config, pairs, user_logger, cycle_count):
    ...
    for pair in pairs:
        ...
        if not has_open_position:
            ...
            elif not is_trending and current_price >= last_donch_mid and current_price >= last_fib_382 and pnl_pct > 0.4:
                signal = "sell"
                reason = (
                    f"Take profit de rango por confluencia Donchian+Fibo: "
                    f"precio recuperó zona media/alta con {pnl_pct:.2f}%."
                )
            ...
            elif pnl_pct > 2.0 and current_price >= last_donch_upper and last_vol_balance < 0.95:
                signal = "sell"
                reason = (
                    f"Salida de distribución: precio en techo Donchian con debilidad de volumen comprador "
                    f"(balance={last_vol_balance:.2f}), asegurando +{pnl_pct:.2f}%."
                )
            else:
