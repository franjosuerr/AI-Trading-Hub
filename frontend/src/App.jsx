...
function Dashboard({ userRole, userId, username, onLogout }) {
  ...
  const [globalConfig, setGlobalConfig] = useState({
    ...
    order_amount_per_pair: 'BTC/USDT:0.001,ETH/USDT:0.01,SOL/USDT:1', pair_delay: 2, max_trades_per_day: 5, stop_loss_percent: 2.0, log_level: 'INFO'
  });
        ...
        <div className="modal-overlay">
          <div className="card modal-content" style={{ width: '95%', maxWidth: '750px', padding: '2rem', background: 'var(--bg-dark)', border: '1px solid rgba(112,0,255,0.3)' }}>
            ...
            <form onSubmit={handleSaveGlobal} style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxHeight: '70vh', overflowY: 'auto', paddingRight: '10px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>MONTOS POR PAR</label>
                <input value={globalConfig.order_amount_per_pair} onChange={(e) => setGlobalConfig({ ...globalConfig, order_amount_per_pair: e.target.value })} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
